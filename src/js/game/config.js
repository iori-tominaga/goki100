...
export const CONFIG = {
  ...
  breeding: {
    ...
    // 巣：壁に空いた穴の奥（＝プレイエリアの外）。だから餌もゴキも湧かない。
    // 連れて行った仲間は穴に潜り、どの危険でも死ななくなる。
    // ただし巣の中では餌を拾わないので、入れすぎると増殖が止まる。
    // wall … どの壁に穴を開けるか（'x-' 左, 'x+' 右, 'z-' 奥, 'z+' 手前）。
    // along … その壁に沿った位置。depth … 壁の外側へどれだけ掘るか。
    nest: {
      radius: 4.5,
      entrance: 5.5,   // 連れて来た仲間がこの距離まで穴口に近づくと潜り込む
                       // （隊列は押し合いで穴口に密集しきれないので広めに取る）
      depth: 7,        // 壁の外へ掘る奥行き
      spots: [
        { wall: 'x-', along: 12 },
        { wall: 'x+', along: -6 },
    ...
    // 連れて行き：近くの仲間の前で「つれていく」を押すと隊列に加わり、
    // 以後プレイヤーの後を追う。もう一度押すと隊列を解散する。
    recruit: { showRadius: 6, grabRadius: 7, followSpeed: 8.5, followGap: 2.2 },
  },
